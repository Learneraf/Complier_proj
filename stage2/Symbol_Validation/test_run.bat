...
:: 语法文件名
set "MAIN_CLASS=Validate"
:: 主类名
set "TEST_FILE1=test_error.c"
set "TEST_FILE2=test_complex.c"
:: 两个测试文件的名称
...
if %errorlevel% neq 0 (
    echo 错误：生成解析器代码失败！请检查jar包路径和Symbol.g4是否存在。
    pause
    exit /b 1
)
echo 解析器代码生成成功！
...
if %errorlevel% neq 0 (
    echo 错误：编译Java文件失败！请检查Validate.java是否存在。
    pause
    exit /b 1
)
echo Java文件编译成功！
